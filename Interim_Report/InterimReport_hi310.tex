...
In this section we will outline the work that is to be done in the remainder of the project as well as the work that has already been completed. We provide a set of milestones and additionally an estimated timescale when the work should be completed. Moreover, we discuss fallback positions in case any piece of work can not be achieved as planned.
...
\begin{itemize}
	...
	We need to finish implementing the message passing community detection algorithms, so that we can apply them to the synthetic data and compare.
	...
	We wish to investigate more complex models for crowdsourcing systems, such as a more general worker distribution, and their effects on the algorithms we have studied. We will seek mathematical justifications as well as evidence from applications to synthetic data, implemented from the new model.
