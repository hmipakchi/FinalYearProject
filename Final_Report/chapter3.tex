...
This algorithm is commonly known as the \emphT{Louvain method} in the literature, and we shall also often refer to the algorithm by this name.
...
The belief propagation (BP) algorithm of \cite{Has06,DKM+13} is designed to infer the group assignment from an instance of the planted partition model outlined in \cref{subsec:plantedPartitionModel}.
...
\begin{equation}
	...
	\probability{\{q_{i}\} \given \graphvar{G},\theta} = \frac{\probability{\graphvar{G},\{q_{i}\} \given \theta}}{\sum_{t_{i}} \probability{\graphvar{G},\{t_{i}\} \given \theta}}
\end{equation}
where $t_{i}$ represent the ground-truth node assignments.
We briefly make an important connection between this problem and statistical physics.
The \emphT{Boltzmann distribution} of a generalised Potts model (consider simply $\{ \sigma \}$ as a set of discrete variables) with Hamiltonian $H(\{ \sigma \})$ at inverse temperature $\beta$  \cite{DKM+13,Pru14a,Pru14b,Sus} is given by
\begin{equation}
	\label{eq:boltzmannDistributionPottsModel}
	\mu(\{ \sigma \}) = \frac{\euler^{- \beta H(\{ \sigma \})}}{\sum_{\{ \sigma \}} \euler^{- \beta H(\{ \sigma \})}}
\end{equation}
where each assignment of the variable $\{ \sigma \}$ has a weight $\euler^{- \beta H(\{ \sigma \})}$ known as the \emphT{Boltzmann weight} and $Z(\beta) = \sum_{\{ \sigma \}} \euler^{- \beta H(\{\sigma\})}$ is called the \emphT{partition sum} (i.e. the sum of the Boltzmann weights over all possible configurations).
We know realise that \cref{eq:probabilityDistributionGroupAssignments} corresponds to a Boltzmann distribution of a generalised Potts model at unit temperature (i.e. $\beta = 1$) with Hamiltonian \cite{DKM+13} given by
\begin{equation}
...
\end{equation}
where the estimated group assignments $\{q_{i}\}$ represent the discrete variables $\{ \sigma \}$.
The corresponding Boltzmann distribution \cite{DKM+13} is
\begin{equation}
	\label{eq:boltzmannDistributionGroupAssignments}
	\mu(\{q_{i}\} \given \graphvar{G}, \theta) = \probability{\{q_{i}\} \given \graphvar{G},\theta}  = \frac{\euler^{- H(\{q_{i}\} \given \graphvar{G}, \theta)}}{\sum_{\{q_{i}\}} \euler^{- H(\{q_{i}\} \given \graphvar{G}, \theta)}}
\end{equation}
and the corresponding partition \cite{DKM+13} is
\begin{equation}
	\label{eq:partitionFunctionGroupAssignments}
	Z(\graphvar{G}, \theta) = \sum_{\{q_{i}\}} \euler^{- H(\{q_{i}\} \given \graphvar{G}, \theta)}
\end{equation}
...
\end{equation}
where $N(i)$ denotes the neighbourhood of node $i$ and $Z^{i \rightarrow j}$ is a constant that normalises the messages ensuring they form a probability distribution (i.e. so that $\sum_{u_{i}} m_{u_{i}}^{i \rightarrow j} = 1$) \cite{DKM+13}.
