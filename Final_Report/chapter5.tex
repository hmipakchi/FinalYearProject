...
Given the conclusions of our analysis in \cref{sec:cha:communityDetectionAlgorithms}, and that the real-world data in this case is not well represented by generative random models, we shall use the modularity optimisation as a basis of some of our algorithms, with spectral clustering also considered as a baseline method. 
...
We move on to understand the potential issues with the naive approach hinted in the previous section. Intuitively, the term $k_{i}k_{j}/2m$ reflects the null hypothesis that the observed network structure is wholly based on the degrees of the nodes.
...
The quantities $k_{i} \equiv \sum_{j} C_{ij}$ and $2m \equiv \sum_{ij} C_{ij}$ do not make up a clear notion of a null model when combined.
...
The spectral clustering algorithm is based on the exact same idea considered for binary networks considered in \cref{subsec:spectralClustering}, but we shall use the filtered correlation matrix, $\matvar{C}^{(g)}$, as input.
