...
Investors tend not to consider the prices of assets they have invested in, but rather the \emphT{return} generated.
Let us consider a financial asset whose price at time $t$ is $p(t)$.
One popular measure of return is called the \emphT{rate of return} \cite{Kuh12e,BKM13} at a time $t$, denoted by $r(t)$, which is defined as
\begin{equation}
...
Critically, the rate of return is sensitive to large changes for longer time horizons \cite{Onn02}.
...
This process is often called \emphT{compounding}.
The time at which the interest is compounded is called the \emphT{compounding period}.
\emphT{Compound interest} involves interest being paid on both the principal and the accumulated interest up to the present \cite{Onn02}.
...
Firstly, if we assume asset prices have a \emphT{log normal} distribution, then the logarithmic returns are conveniently normally distributed \cite{QuaWp}.
