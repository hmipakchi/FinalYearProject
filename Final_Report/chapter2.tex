...
\begin{definition}
	...
	The matrix $\widetilde{\matvar{L}} = \matvar{I}  - \matvar{D}^{-1/2}\matvar{A}\matvar{D}^{-1/2}$ is called the normalised Laplacian matrix, where $\matvar{I}$ is the $n \times n$ identity matrix.
\end{definition}
...
The techniques used to prove these results are beyond the scope of the project, however these results provide a very important limit on the ability of algorithms to detect communities.
...
Also, we are interested in the regime where the size of these communities is small relative to the size of the graph (and are not necessarily of the same size either).
...
We have essentially represented the problem of inferring the hidden community from the graph by a problem of estimating a rank-1 matrix in noise. Note that for the generalisation with $k$ communities, we would get a rank-k matrix plus noise for the normalised adjacency matrix.
