...
We shall revisit modularity optimisation algorithms as they are useful for the particular application of financial networks, further studied in \cref{cha:communityDetectionFinancialNetworks,cha:temporalEvolutionFinancialNetworks}.
...
%---   TODO
%%% need to describe empirical results for Belief Propagation Algorithm.
We wish to find a class of algorithms that, empirically, seem to reliably detect communities up to the phase transition in the sparse regime as well as the dense regime.
Unfortunately, both the spectral clustering and modularity optimisation methods could not achieve this, but we are hopeful that the BP algorithm from \cref{sec:beliefPropagationAlgorithm} given its derivation.
